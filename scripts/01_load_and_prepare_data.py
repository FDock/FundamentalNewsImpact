...
# --- Load tick data ---
try:
    tick_path = os.path.join(RAW_DATA_DIR, "tick_data.csv")
    tick_df = pd.read_csv(tick_path, names=["datetime", "bid", "ask"])
...
try:
    ...
    # Clean column names
    news_df.columns = [col.strip().lower().replace(" ", "_") for col in news_df.columns]
